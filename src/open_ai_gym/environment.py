...
from src.generator import sin_gaussian


class SkippyEnv(Env):
    metadata = {'render.modes': ['human'], 'render_fps': 30}

    def __init__(self, generator: Generator[float, None, None] = sin_gaussian(step=np.pi / 32, sigma=1e-1), eps=1e-5):
        super(SkippyEnv, self).__init__()
        self.generator = generator
        self.eps = eps
        self.action_space = spaces.Box(low=-1e7, high=1e7, shape=(1,), dtype=np.float64)
        self.observation_space = spaces.Box(low=-1e7, high=1e7, shape=(1,), dtype=np.float64)
        ...
        self.diff_plot, = self.axs[0].plot([0], [0], label='Prediction - Conceived')
    ...
    def render(self, mode="human"):
        diff_stats = np.append(self.diff_plot.get_ydata(), np.abs(self.last_diff))[-self.plot_length:]
        self.diff_plot.set_ydata(diff_stats)
        self.diff_plot.set_xdata(list(range(len(diff_stats))))

        prediction_stats = np.append(self.prediction_plot.get_ydata(), self.last_guess)[-self.plot_length:]
        self.prediction_plot.set_ydata(prediction_stats)
        self.prediction_plot.set_xdata(list(range(len(prediction_stats))))

        conceived_stats = np.append(self.conceived_plot.get_ydata(), self.current_value)[-self.plot_length:]
        ...
        self.axs[1].set_ylim((
            min(np.min(prediction_stats[-self.plot_length // 10:]), np.min(conceived_stats)),
            max(np.max(prediction_stats[-self.plot_length // 10:]), np.max(conceived_stats))
        ))
